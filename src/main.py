"""
main.py - GA-based Exam Timetabling System
--------------------------------------------
This is the main module that orchestrates the loading of data,
execution of the genetic algorithm, and saving of results.
"""
...
def save_results(dataset_name: str, result: Dict, history: list, output_dir: str = None):
    if output_dir is None:
        output_dir = os.path.join(get_project_root(), "results")
    ...
    with open(slot_file, 'w') as f:
        ...
        f.write(f"# Violations: {result['details']['hard_violations']}\n")
        ...
        for slot_num in sorted(slot_exams.keys()):
            exams = slot_exams[slot_num]
            f.write(f"Slot {slot_num + 1}: {' '.join(map(str, sorted(exams)))}\n")
    ...
    with open(history_file, 'w') as f:
        ...
        for h in history:
            f.write(f"{h['generation']+1},{h['best_fitness']:.6f},{h['avg_fitness']:.6f},"
                    f"{h['slots']},{h['hard_violations']}\n")
...
def main():
    print("="*60)
    print("CENG482 - Exam Scheduling Genetic Algorithm")
    print("="*60)
